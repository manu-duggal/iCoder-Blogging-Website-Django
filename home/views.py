...
def search(request):
    ...
    if allPosts.count() == 0:
        messages.warning(request, 'No Search result Found! please review your search query.')
...
def handleSignup(request):
    if request.method == 'POST':
        ...
        # checks for erronous inputs
        
        # username should be less than 10 characater
        if len(username) > 10:
            messages.error(request, 'Username must be under 10 characters')
            return redirect('home')

        # username should be alphanumberic
        if not username.isalnum():
            messages.error(request, 'Username Should only contains letters and numbers')
            return redirect('home')

        # both password filed should be same
        if pass1 != pass2:
...
def handleLogin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'Invalid Crentials, please try again!')
            return redirect('home')
